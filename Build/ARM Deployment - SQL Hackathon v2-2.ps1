...
Invoke-WebRequest 'https://github.com/markjones-msft/SQL-Hackathon/blob/master/Build/SQL%20SSIS%20Databases/TenantDataDb.bak?raw=true' -OutFile "$env:temp\TenantDataDb.bak" | Wait-Process

Set-AzStorageBlobContent  -Container "sqlbackups" -File "$env:temp\LocalMasterDataDb.bak" -Blob "LocalMasterDataDb.bak" -context $Context -force
