...
#Start-Process -file 'C:\Install\SSDT-Setup-ENU.exe' -arg '/layout c:\Install\vs_install_bits /quiet /log C:\Install\SSDTLayout_install.txt' | Out-Null
...
$exitCode = [Diagnostics.Process]::Start('C:\Install\vs_sql.exe','--layout c:\install\vs_install_bits --lang en-us --quiet --log C:\Install\VSLayout_install.txt').WaitForExit()
$exitCode = [Diagnostics.Process]::Start('C:\Install\SSDT-Setup-ENU.exe','/layout c:\Install\vs_install_bits /quiet /log C:\Install\SSDTLayout_install.txt').WaitForExit()

$exitCode = [Diagnostics.Process]::Start('C:\Install\vs_install_bits\vs_setup.exe','--noweb --quiet').WaitForExit()
$exitCode = [Diagnostics.Process]::Start('C:\Install\vs_install_bits\SSDT-Setup-enu.exe','/install INSTALLIS /quiet /norestart /log C:\Install\SSDT_install.txt').WaitForExit()
